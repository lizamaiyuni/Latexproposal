...
\begin{enumerate}[1.]
	\item Peneliti
	      \newline Pengguna yang menggunakan Aplikasi Mapping berbasis Android untuk mempermudah pemetaan kekuatan sinyal atau nilai RSSI dari alat yang disebut Beacon.
	\item Penguji Aplikasi
	      \newline Pengguna yang menggunakan Aplikasi SpecifyLocation berbasis Android untuk mengetahui posisi dimana pengguna berada saat ini di gedung A FMIPA USK.
\end{enumerate}

\subsection{Use Case Diagram}
\textit{Use case diagram} digunakan untuk mewakili beberapa korelasi atau beberapa daftar tindakan antara elemen dari sebuah sistem dengan aktor. \textit{Use case diagram} digunakan untuk merepresentasikan hubungan dan ketergantungan dalam sebuah sistem. Aktor bisa berupa manusia atau fitur dari sistem itu sendiri, berguna untuk menentukan peran yang dimiliki oleh pengguna atau sistem lain yang saling berinteraksi \citep{Syed2018}.
\textit{Use Case Diagram} dari sistem yang telah dibangun dapat dilihat pada Gambar \ref{usecasemapping} dan Gambar \ref{usecasepenguji}.

\begin{figure}[H]
	\center
	\shadowbox
	{\includegraphics [width=10cm, height=9cm]{gambar/mapping}}
	\caption{\textit{Use Case Diagram} Aplikasi Mapping.}
	\label{usecasemapping}
\end{figure}

\par Gambar \ref{usecasemapping} diatas menjelaskan aktivitas-aktivitas yang dapat dilakukan oleh peneliti saat sedang menggunakan aplikasi \textit{mapping}. Ketika peneliti membuka aplikasi, akan terlihat halaman utama dari aplikasi \textit{Mapping}. Selanjutnya, ketika peneliti memindai sinyal bluetooth, peneliti diminta untuk mengizinkan aplikasi untuk mengakses bluetooth pada perangkat yang digunakan. kemudian, peneliti bisa memindai kekuataan sinyal atau RSSI dari beacon. Setelah itu, hasil dari pemindaian kekuataan sinyal atau RSSI tersebut dapat disimpan dan peneliti dapat melihat jumlah data yang sudah disimpan di server.
...
\begin{figure}[H]
	...
	\caption{\textit{Use Case Diagram} Aplikasi SpecifyLocation.}
	\label{usecasepenguji}
\end{figure}

\par Gambar \ref*{usecasepenguji} menjelaskan aktivitas-aktivitas yang dapat dilakukan oleh penguji aplikasi saat sedang menggunakan apabila SpecifyLocation. Sama halnya dengan aplikasi \textit{mapping}, ketika penguji aplikasi membuka aplikasi, akan terlihat halaman utama dari aplikasi SpecifyLocation. Selanjutnya ketika penguji memprediksi lokasi, penguji diminta untuk mengizinkan aplikasi untuk mengakses bluetooth pada perangkat yang sedang digunakan. Setelah itu, dari hasil memprediksi lokasi akan ditampilkan prediksi lokasi pengguji saat ini di dalam gedung FMIPA USK, serta penguji dapat melihat estimasi jumlah orang yang sedang berada di dalam gedung FMIPA USK dengan syarat semua orang yang berada di dalam gedung menggunakan aplikasi SpecifyLocation.
	...
	\item Antar Muka Aplikasi SpecifyLocation
	      ...
	      \begin{figure} [H]
		      ...
		      \begin{subfigure}{.5\textwidth}
			      \centering
			      % include second image
			      \includegraphics[width=.5\linewidth]{gambar/Lantai1.png}
		      ...
		      \begin{center}
			      \begin{subfigure}{.5\textwidth}
				      \centering
				      % include third image
				      \includegraphics[width=.5\linewidth]{gambar/Lantai2.png}
...
\begin{enumerate}[a.]
	\item Aplikasi \textit{Mapping}
	      \\ Aplikasi ini adalah sebuah aplikasi \textit{mobile} berbasis Android yang dibuat menggunakan bahasa pemrograman JavaScript dan \textit{framework} React Native. Media untuk penyimpanan data pada aplikasi \textit{mapping} ini menggunakan \textit{database} MongoDb Compass yang datanya akan dikirim melalui API [Isi nanti]. Potongan kode program yang terdapat dalam aplikasi \textit{mapping} ini dapat dilihat pada Progran 4.1 berikut ini.
	      \vspace{0.4cm}
